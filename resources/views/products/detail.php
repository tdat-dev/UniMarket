        ...
        <div class="bg-white rounded-sm shadow-sm p-4">
            <div class="grid grid-cols-1 md:grid-cols-12 gap-8">
                ...
                <div class="md:col-span-5">
                    <div class="relative w-full aspect-square bg-gray-100 rounded-sm overflow-hidden border border-gray-200">
                    ...
                    <div class="flex gap-2 mt-4 overflow-x-auto">
                        <?php for($i=0; $i<5; $i++): ?>
                        <div class="w-20 h-20 border border-gray-200 hover:border-[#2C67C8] cursor-pointer rounded-sm overflow-hidden <?= $i===0 ? 'border-[#2C67C8]' : '' ?>">
                            <img src="/uploads/<?= htmlspecialchars($product['image']) ?>" class="w-full h-full object-cover">
                        </div>
                        <?php endfor; ?>
                    </div>
                </div>
                    ...
                    <div class="space-y-4 text-sm text-gray-600">
                        ...
                        <div class="flex items-center">
                            ...
                            <div class="flex items-center border border-gray-300 rounded-sm">
                                <button type="button" id="btn-decrease" class="px-3 py-1 border-r border-gray-300 hover:bg-gray-50 min-w-[32px]">-</button>
                                <input type="number" id="input-quantity" value="1" min="1" max="<?= $product['quantity'] ?>" class="w-14 text-center outline-none bg-white font-medium [appearance:textfield] [&::-webkit-outer-spin-button]:appearance-none [&::-webkit-inner-spin-button]:appearance-none">
                                <button type="button" id="btn-increase" class="px-3 py-1 border-l border-gray-300 hover:bg-gray-50 min-w-[32px]">+</button>
                            </div>
                    ...
                    <div class="flex gap-4 pt-4">
                        <?php if ($product['quantity'] > 0 && $product['status'] === 'active'): ?>
                            <form action="/cart/add" method="POST" class="flex gap-4 w-full">
                                ...
                                <button type="submit" name="action" value="add" class="flex-1 px-6 py-3 border border-[#EE4D2D] bg-[#EE4D2D]/10 text-[#EE4D2D] font-medium rounded-sm hover:bg-[#EE4D2D]/20 transition-colors flex items-center justify-center gap-2">
                                    <i class="fa-solid fa-cart-shopping"></i> Thêm vào giỏ hàng
                                </button>
                                <button type="submit" name="action" value="buy" class="flex-1 px-8 py-3 bg-[#EE4D2D] text-white font-medium rounded-sm hover:bg-[#D73211] transition-colors shadow-sm text-center">
                                    Mua ngay
                                </button>
                            </form>
                        <?php else: ?>
                            <button disabled class="w-full px-8 py-3 bg-gray-300 text-gray-500 font-medium rounded-sm cursor-not-allowed">
        ...
        <div class="bg-white rounded-sm shadow-sm p-4">
            <div class="flex items-center gap-4">
                ...
                <div class="flex gap-2">
                    <a href="/chat?user_id=<?= $seller['id'] ?>" class="px-4 py-2 border border-[#2C67C8] text-[#2C67C8] rounded-sm hover:bg-blue-50 font-medium text-sm flex items-center gap-1">
                        <i class="fa-regular fa-comment-dots"></i> Chat ngay
                    </a>
                    <a href="/shop?id=<?= $seller['id'] ?>" class="px-4 py-2 bg-gray-100 text-gray-600 rounded-sm hover:bg-gray-200 font-medium text-sm flex items-center gap-1">
                        ...
                        <div class="group bg-white border border-transparent hover:border-[#2C67C8] hover:shadow-md transition-all duration-200 rounded-sm overflow-hidden relative">
                            <a href="/product-detail?id=<?= $item['id'] ?>" class="block">
                                ...
                                <div class="relative pt-[100%] overflow-hidden bg-gray-100">
                                    ...
                                    <div class="absolute top-0 left-0 bg-[#00bfa5] text-white text-[10px] font-bold px-1.5 py-0.5 rounded-br-sm">Freeship</div>
                                </div>
                                ...
                                <div class="p-2 space-y-1">
                                    ...
                                    <div class="flex items-center justify-between pt-2">
                                        <span class="text-[10px] text-gray-400 font-light truncate w-full">TP. Hồ Chí Minh</span>
                                    </div>
                                </div>
                            </a>

                            <!-- Hover Action (Tìm kiếm tương tự) -->
                            <div class="absolute bottom-0 left-0 w-full bg-[#2C67C8] text-white text-center text-xs py-1.5 opacity-0 group-hover:opacity-100 transition-opacity cursor-pointer">
