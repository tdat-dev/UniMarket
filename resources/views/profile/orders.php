...
<main class="bg-gray-50 min-h-screen pb-12">
                                ...
                                <?php foreach ($order['items'] as $item): ?>
                                    <div class="flex gap-4">
                                        ...
                                        <div>
                                             ...
                                             <p class="text-xs font-medium text-gray-700 mt-1"><?= number_format($item['price_at_purchase'] ?? 0, 0, ',', '.') ?>đ</p>
                                        </div>
                                    </div>
                                <?php endforeach; ?>
                            </div>
    ...
    <!-- Cancel Reason Modal -->
    <div id="cancelReasonModal" class="fixed inset-0 bg-black bg-opacity-50 flex items-center justify-center z-50 hidden" style="backdrop-filter: blur(2px);">
        <div class="bg-white rounded-lg p-6 max-w-md w-full mx-4 shadow-xl transform transition-all">
            ...
            <div class="space-y-3 mb-6">
                ...
                <label class="flex items-center gap-3 p-3 border border-gray-200 rounded-lg cursor-pointer hover:bg-gray-50 transition">
                    <input type="radio" name="cancel_reason" value="change_mind" class="w-4 h-4 text-blue-600 focus:ring-blue-500 border-gray-300">
                    <span class="text-sm text-gray-700">Tôi muốn thay đổi địa chỉ/số điện thoại</span>
                </label>
                <label class="flex items-center gap-3 p-3 border border-gray-200 rounded-lg cursor-pointer hover:bg-gray-50 transition">
                    <input type="radio" name="cancel_reason" value="change_product" class="w-4 h-4 text-blue-600 focus:ring-blue-500 border-gray-300">
                    <span class="text-sm text-gray-700">Tôi muốn đổi sản phẩm khác</span>
                </label>
                <label class="flex items-center gap-3 p-3 border border-gray-200 rounded-lg cursor-pointer hover:bg-gray-50 transition">
                    <input type="radio" name="cancel_reason" value="too_long" class="w-4 h-4 text-blue-600 focus:ring-blue-500 border-gray-300">
                    <span class="text-sm text-gray-700">Thời gian giao hàng quá lâu</span>
                </label>
                 <label class="flex items-center gap-3 p-3 border border-gray-200 rounded-lg cursor-pointer hover:bg-gray-50 transition">
                    <input type="radio" name="cancel_reason" value="not_needed" class="w-4 h-4 text-blue-600 focus:ring-blue-500 border-gray-300">
                    <span class="text-sm text-gray-700">Tôi không còn nhu cầu mua nữa</span>
                </label>
                 <label class="flex items-center gap-3 p-3 border border-gray-200 rounded-lg cursor-pointer hover:bg-gray-50 transition">
                    <input type="radio" name="cancel_reason" value="other" class="w-4 h-4 text-blue-600 focus:ring-blue-500 border-gray-300">
                    <span class="text-sm text-gray-700">Lý do khác</span>
                </label>
            </div>
            <div class="flex justify-end gap-3 pt-2">
                <button onclick="closeCancelModal()" class="px-4 py-2 text-gray-700 hover:bg-gray-100 rounded-md text-sm font-medium">Đóng</button>
                <button onclick="proceedToConfirmation()" class="px-6 py-2 bg-blue-600 text-white rounded-md hover:bg-blue-700 text-sm font-medium shadow-sm transition">Xác nhận</button>
            </div>
        </div>
    </div>

    <!-- Final Confirmation Modal -->
     <div id="confirmCancelModal" class="fixed inset-0 bg-black bg-opacity-50 flex items-center justify-center z-50 hidden" style="backdrop-filter: blur(2px);">
        <div class="bg-white rounded-lg p-6 max-w-sm w-full mx-4 text-center shadow-xl">
            ...
            <p class="text-gray-500 mb-6 text-sm leading-relaxed">Bạn có chắc chắn muốn hủy đơn hàng này không? Hành động này không thể hoàn tác.</p>
            <div class="flex justify-center gap-3">
                <button onclick="closeConfirmModal()" class="px-5 py-2.5 border border-gray-300 rounded-md text-gray-700 font-medium hover:bg-gray-50 text-sm transition">Không</button>
                <form id="finalCancelForm" action="/profile/orders/cancel" method="POST" class="inline">
                    ...
                    <button type="submit" class="px-5 py-2.5 bg-red-600 text-white rounded-md font-medium hover:bg-red-700 text-sm shadow-sm transition" style="background-color: #dc2626;">Đồng ý hủy</button>
                </form>
            </div>
        </div>
    </div>
    ...
    <div id="rebuyOrderModal" class="fixed inset-0 bg-black bg-opacity-50 flex items-center justify-center z-50 hidden" style="backdrop-filter: blur(2px);">
        <div class="bg-white rounded-lg p-6 max-w-sm w-full mx-4 text-center shadow-xl">
            ...
            <p class="text-gray-500 mb-6 text-sm leading-relaxed">Bạn có đồng ý mua lại các sản phẩm trong đơn hàng này không?</p>
            <div class="flex justify-center gap-3">
                <button onclick="closeRebuyModal()" class="px-5 py-2.5 border border-gray-300 rounded-md text-gray-700 font-medium hover:bg-gray-50 text-sm transition">Quay lại</button>
                <form id="rebuyOrderForm" action="/profile/orders/rebuy" method="POST" class="inline">
                    ...
                    <button type="submit" class="px-5 py-2.5 bg-blue-600 text-white rounded-md font-medium hover:bg-blue-700 text-sm shadow-sm transition">Đồng ý</button>
                </form>
            </div>
        </div>
    </div>
